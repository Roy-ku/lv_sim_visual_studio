...
int main()
{
     ...
     */
     ...
     lv_fs_dir_t d;
     lv_fs_res_t ret = 0;
     ret = lv_fs_dir_open(&d, "0:");
     if (ret == LV_FS_RES_OK)
     {
         char b[MAX_PATH];
         memset(b, 0, MAX_PATH);
         uint8_t ret = 0;
         ret = lv_fs_dir_read(&d, b);
         if (ret == LV_FS_RES_OK)
         {
             printf("%s\n", b);
         }
     ...
     // ----------------------------------
    lv_pm_init();
    lv_pm_page_t* page_starup = lv_pm_create_page(0);
    page_starup->onLoad = InitPageStartup;
    page_starup->unLoad = DeInitPageStartup;
    lv_pm_page_t* page1 = lv_pm_create_page(1);
    page1->onLoad = InitPages1;
    page1->didAppear = didAppearPages1;
    page1->unLoad = DeInitPages1;
    lv_pm_page_t* page2 = lv_pm_create_page(2);
    page2->onLoad = InitPages2;
    page2->unLoad = DeInitPages2;
    lv_pm_page_t* page3 = lv_pm_create_page(3);
    page3->onLoad = InitPages3;
    page3->unLoad = DeInitPages3;
